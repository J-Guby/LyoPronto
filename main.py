...
vial = dict([('Av',3.8),('Ap',3.14),('Vfill',2.0)]) 
...
if sim['tool'] == 'Freezing Calculator':
	product = dict([('cSolid',5.0),('Tpr0',0),('Tf',0),('Tn',0)]) 
elif not(sim['tool'] == 'Primary Drying Calculator' and sim['Rp_known'] == 'N'):
	product = dict([('cSolid',5.0),('R0',1.4),('A1',16.0),('A2',0.0)]) 
else:
	product = dict([('cSolid',5.0)]) 
...
if sim['tool'] == 'Design-Space-Generator':
	...
	Tshelf = dict([('init',-5.0),('setpt',[20.0]),('ramp_rate',1.0)]) 
	...
	# ramp_rate = Shelf temperature ramping rate in C/min
	Tshelf = dict([('init',-5.0),('setpt',[20.0]),('dt_setpt',[1800.0]),('ramp_rate',1.0)]) 
else:
	# Shelf temperature limits in C
	Tshelf = dict([('min',0),('max',0)]) 
...
if not(sim['tool'] == 'Freezing Calculator' and sim['tool'] == 'Primary Drying Calculator'):
	eq_cap = dict([('a',-0.182),('b',11.7)]) 

# Equipment load
if not(sim['tool'] == 'Freezing Calculator' and sim['tool'] == 'Primary Drying Calculator'):
	nVial = 398.0	# Number of vials 
